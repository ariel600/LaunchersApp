...
export async function getLaunchersController(req, res) {
    try {
        const launchers = await getLaunchersService()
...
export async function createLauncherController(req, res) {
    try {
        const { name, rocketType, latitude, longitude, city } = req.body
        const result = await createLauncherService({ name, rocketType, latitude, longitude, city })
